...
func newListener(ln quicreuse.Listener, t *transport, localPeer peer.ID, key ic.PrivKey, rcmgr network.ResourceManager) (listener, error) {
	...
	if t.ech.enabled {
		if _, err := t.currentECHConfigList(); err != nil {
			return listener{}, err
		}
		t.startECHPublisher()
	}
...
func (l *listener) multiaddrs(version quic.Version) []ma.Multiaddr {
	addr, ok := l.localMultiaddrs[version]
	if !ok {
		return nil
	}
	...
	if err != nil {
		log.Error("failed to build ech listen address", "err", err)
		return addrs
	}
	return append(addrs, echAddr)
}
