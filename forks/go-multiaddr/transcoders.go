...
// TranscoderECH transcodes a TLS Encrypted Client Hello config (an
// ECHConfigList as defined in the TLS ECH draft). The binary form is the raw
// ECHConfigList bytes; the string form is the multibase (base64url) encoding of
// those bytes.
...
func validateECH(b []byte) error {
	// An ECHConfigList is a uint16-length-prefixed list of ECHConfig entries.
	// We only validate the outer framing here; the TLS stack validates the
	// individual configs.
	if len(b) < 2 {
		return fmt.Errorf("ech config too short")
	}
	l := int(b[0])<<8 | int(b[1])
	if l != len(b)-2 {
